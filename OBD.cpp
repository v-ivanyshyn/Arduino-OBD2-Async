...
// async:
void COBD::asyncRequest(byte pid)
{
    if (aInProgress)
    {
        while(this->available())                // need to complete previous request
            this->read();
    }
    aInProgress = true;
    aPid = pid;
...
bool COBD::asyncInProgress()
{
    return aInProgress;
}

bool COBD::asyncGet(int& result)
{
    if (!aInProgress)
        return false;
    ...
    while(this->available())
    {
        ...
        if ((aBufferN > 2) && (c == '>'))        // prompt char received
        {   // all data received, filter buffer:
            aBuffer[aBufferN] = 0;
            char *p = aBuffer;
            while ((p = strstr(p, "41 ")))     // common prefix
            {
                p += 3;
                byte pid = hex2uint8(p);       // 2 hex digits of PID
                if (pid == aPid)
                {
                    p += 2;
                    if (*p == ' ')
            ...
            result = normalizeData(aPid, p);
            aInProgress = false;	                
            return true;
        }
        else if (aBufferN < (OBD_RECV_BUF_SIZE - 1))
        {
            if ((c == '.') && (aBufferN > 2) && (aBuffer[aBufferN - 1] == '.') && (aBuffer[aBufferN - 2] == '.'))
                aBufferN = 0;
            else
            {
                aBuffer[aBufferN] = c;
...
int COBD::asyncDuration()
{
    if (!aInProgress)
        return 0;
    return millis() - aTimestamp;
}
